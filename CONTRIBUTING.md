...
Use Node.js 22 or 24 and npm. If you use nvm, run `nvm use`; `.nvmrc` selects Node.js 24. The project has no runtime or development dependencies to install. The checked-in `demo/index.html` is ready to open directly in a browser.
...
Double-click `demo/index.html` for the self-contained demo. No local server is needed.

After changing `claper-wordcloud.js`, `demo/words.js`, `LICENSE`, or `demo/template.html`, run `npm run build:demo` and commit the updated `demo/index.html`. The generator embeds the source, safely serializes sample text, and includes the full license. Do not hand-edit the generated copy of the library. `npm run check:demo` fails if the output is stale, and CI runs this check.
...
| `demo/build.js`, `demo/index.html` | Dependency-free generator and its checked-in, self-contained HTML output. |
| `demo/words.js` | Sample dataset embedded in the standalone HTML by the generator. |
| `demo/assets/` | Word cloud images used in the README. |
...
- Word content goes through `textContent` or DOM attribute assignment, never HTML interpolation.
...
Use the standalone demo and browser-based library examples to check rendering and interactions:

- Open a copy of `demo/index.html` in a separate directory with no other files, using `file://` and with network access disabled. All controls must work without resource requests.
- Add a new word, repeat it, clear the cloud, and restore the demo. Counts and weights should remain correct.
...
npm test          # Public API and lifecycle regressions.
...
The `CI` workflow publishes `@claperco/wordcloud.js` after a push to `main`, but only after both verification jobs pass. Pull requests, `dev`, other branches, and tags never publish. **Run workflow** on `main` can retry a release without another commit; selecting `dev` runs checks only.
...
1. Use [ClaperCo/wordcloud.js](https://github.com/ClaperCo/wordcloud.js) with `https://github.com/ClaperCo/wordcloud.js.git` as the `origin` remote. Keep `main` as the default branch. Configure the publishing environment below before the first push.
...
5. Once the package exists, open its npm **Settings → Trusted Publisher** and choose **GitHub Actions**. Enter organization **`ClaperCo`**, repository **`wordcloud.js`**, workflow filename **`ci.yml`** (not its full path), and environment **`npm`**. Enable direct **`npm publish`** in the allowed actions; stage-only permission does not permit this workflow to publish.
...
The npm package is **`@claperco/wordcloud.js`**; its GitHub repository is **`ClaperCo/wordcloud.js`**. `package.json` declares the repository, homepage, and issue tracker URLs, and CI publishes that metadata without rewriting it. Enable private vulnerability reporting on GitHub or provide a private contact in `SECURITY.md` before the first public release.
...
3. Run `npm pack`. Its `prepack` hook regenerates the standalone demo and runs verification before creating the archive. Inspect the listed files: the package contains the runtime module, self-contained HTML demo, README image assets, package metadata, license, and documentation, not the template, generator, sample data source, or tests.
4. Install that archive into a small consumer project, import `createWordCloud` by package name, and bundle and run a browser example. Open the installed package's `demo/index.html` directly from disk as well.
